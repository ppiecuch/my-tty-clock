...
constexpr cast_t check_cast(const check_t in) { // Ensure the two types are integers, can't handle floats/doubles
	static_assert(std::numeric_limits<cast_t>::is_integer, "The casting type must be an integer type");
	static_assert(std::numeric_limits<check_t>::is_integer, "The argument must be an integer type");
...
static const box_drawing_set_t BoxDrawingSetRegular = {
...
static const box_drawing_set_t BoxDrawingSetLight = {
	0x2502, // 0xb3 - '│'
	0x2524, // 0xb4 - '┤'
...
const uint8_t DecodeThresholdNonAscii = 0b1'000'0000;
const uint8_t DecodeThreshold2Bytes = 0b1'100'0000;
const uint8_t DecodeThreshold3Bytes = 0b1'110'0000;
const uint8_t DecodeThreshold4Bytes = 0b1'111'0000;
const uint8_t DecodeThreshold5Bytes = 0b1'111'1000;
const uint8_t DecodeThreshold6Bytes = 0b1'111'1100;
const uint16_t EncodeThreshold2Bytes = 0x0080;
const uint16_t EncodeThreshold3Bytes = 0x0800;

// Use the character below if there is no sane way to handle the Unicode glyph
const uint8_t UnknownCharacter = 0x3f; // '?'

// End of file marking, use in some files from unicode.org
const uint8_t EndOfFileMarking = 0x1a;
...
static bool is_combining_mark(const uint32_t code_point) {
	static const std::pair<uint16_t, uint16_t> Ranges[] = {
		// clang-format off
...
const uint8_t ControlCodeDelete = 0x7f;
const uint8_t ControlCodeThreshold = 0x20;

// Unicode code points for screen codes from 0x00 to 0x1f
// see: https://en.wikipedia.org/wiki/Code_page_437
const std::array<uint16_t, 0x20> ScreenCodesWide = {
	// clang-format off
...
const uint16_t ScreenCodeWide7f = 0x2302; // 'del' character
...
static std::optional<uint8_t> grapheme_to_screen_code(const Grapheme &grapheme,
	...
	const auto Begin = ScreenCodesWide.begin();
	const auto End = ScreenCodesWide.end();
...
static wide_string utf8_to_wide(const std::string &str) {
	...
	auto warn_decode_problem = [&already_warned](const size_t position) {
		...
		std::cout << "UNICODE: Problem decoding UTF8 string, position:" << static_cast<unsigned int>(position) << std::endl;
		already_warned = true;
	};
...
static void warn_code_point(const uint16_t code_point) {
	...
	std::cout << "UNICODE: No fallback mapping for code point 0x" << std::hex << code_point << std::endl;
}
...
static void warn_code_page(const uint16_t code_page) {
	...
	std::cout << "UNICODE: Requested unknown code page:" << code_page << std::endl;
}
...
static wide_string dos_to_wide(const std::string &str, const DosStringConvertMode convert_mode, const uint16_t code_page) {
	wide_string str_out = {};
	str_out.reserve(str.size());
...
bool add_if_not_mapped(std::map<T1, T2> &mapping, T1 first, T2 second) {
	[[maybe_unused]] const auto &[item, was_added] = mapping.try_emplace(first, second);
	return was_added;
}
...
static std::ifstream open_mapping_file(const std_fs::path &path_root,
	...
	if (!in_file) {
		std::cerr << "UNICODE: Could not open mapping file " << file_name << std::endl;
	}
...
static void error_parsing(const std::string &file_name, const size_t line_num,
	...
	if (details.empty()) {
		std::cerr << "UNICODE: Error parsing mapping file:" << file_name << ", line:" << static_cast<int>(line_num) << std::endl;
	} else {
		std::cerr << "UNICODE: Error parsing mapping file:" << file_name << ", line:" << static_cast<int>(line_num) << ", details:" << details << std::endl;
	}
}
...
static void error_code_point_found_twice(const uint16_t code_point,
	...
	details << std::string("code point U+") << std::hex << code_point << " found twice";
...
static void error_not_combining_mark(const size_t position,
		const std::string &file_name,
		const size_t line_num) {
	const auto details = std::string("token #") + std::to_string(position) + " is not a supported combining mark";
...
static bool check_import_status(const std::ifstream &in_file,
	...
	if (in_file.fail() && !in_file.eof()) {
		std::cerr << "UNICODE: Error reading mapping file " << file_name;
		return false;
	}

	if (empty) {
		std::cerr << "UNICODE: Mapping file " << file_name << " has no entries" << std::endl;
		return false;
	}
...
static bool check_grapheme_valid(const Grapheme &grapheme, const std::string &file_name, const size_t line_num) {
	if (grapheme.IsValid()) {
		return true;
	}

	std::cerr << "UNICODE: Error, invalid grapheme defined in file " << file_name << ", line:" << static_cast<int>(line_num) << std::endl;
	return false;
}
...
static bool import_mapping_code_page(const std_fs::path &path_root,
	...
	if (!in_file) {
		std::cerr << "UNICODE: Error opening mapping file " << file_name << std::endl;
		return false;
	}
...
static bool construct_mapping(const uint16_t code_page) {
	...
	auto add_to_mappings = [&](const uint8_t first, const Grapheme &second) {
		...
		std::cout << "UNICODE: Mapping for code page " << code_page << " uses a code point twice; character 0x" << std::hex << first << std::endl;
	};
	...
	if (config_mapping.extends_code_page) {
		...
		if (!prepare_code_page(dependency)) {
			std::cerr << "UNICODE: Code page " << code_page << " mapping requires code page " << dependency << " mapping" << std::endl;
			return false;
		}
...
uint16_t get_utf8_code_page() {
	...
	const uint16_t RomCodePage = 437; // United States
	const uint16_t code_page = RomCodePage; // Below EGA it wasn't possible to change the character set
...
static std::string utf8_to_dos_common(const std::string &str, const DosStringConvertMode convert_mode, const UnicodeFallback fallback, const uint16_t code_page) {
	load_config_if_needed();
...
std::string utf8_to_dos(const std::string &str, const DosStringConvertMode convert_mode, const UnicodeFallback fallback) {
	return utf8_to_dos_common(str, convert_mode, fallback, get_utf8_code_page());
}
...
std::string utf8_to_dos(const std::string &str, const DosStringConvertMode convert_mode, const UnicodeFallback fallback, const uint16_t code_page) {
	return utf8_to_dos_common(str, convert_mode, fallback, get_custom_code_page(code_page));
}

static std::string dos_to_utf8_common(const std::string &str, const DosStringConvertMode convert_mode, const uint16_t code_page) {
	load_config_if_needed();
...
std::wstring dos_to_uni(const std::string &str, const DosStringConvertMode convert_mode) {
	return dos_to_uni_common(str, convert_mode, get_utf8_code_page());
}

std::wstring dos_to_uni(const std::string &str, const DosStringConvertMode convert_mode, const uint16_t code_page) {
	return dos_to_uni_common(str, convert_mode, get_custom_code_page(code_page));
}
...
static std::string uppercase_dos_common(const std::string &str, const uint16_t code_page) {
	load_config_if_needed();
